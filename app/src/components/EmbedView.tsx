...
export function EmbedView({
  ...
  const headline = useMemo(() => {
    if (status !== "ready") {
      return message ?? (status === "no-data" ? "Data unavailable for this embed" : "Loading…");
    }
  ...
  return (
    <div className="embed-container" style={{ height: embedParams.height }}>
      {/* Chart area */}
      <div className="embed-chart-wrapper">
        {status === "ready" && projection && projection.length > 0 ? (
          ...
          <div className="h-full flex items-center justify-center">
            <div className="text-sm text-ink-muted">{headline}</div>
          </div>
        )}
      </div>
      ...
      {/* Attribution footer */}
      <EmbedFooter shareUrl={appUrl} chaserName={chaserName} targetName={targetName} />
    </div>
  );
}
