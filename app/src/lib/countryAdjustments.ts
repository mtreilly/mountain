 ...
 */
export function getAdjustment(iso: string, indicatorCode: string): CountryAdjustment | null {
  return (
    COUNTRY_ADJUSTMENTS.find((adj) => adj.iso === iso && adj.indicators.includes(indicatorCode)) ??
    null
  );
}
