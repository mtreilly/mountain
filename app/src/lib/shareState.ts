...
export function parseEmbedParams(search: string): EmbedParams {
  ...
  const embedTheme: EmbedTheme = themeRaw === "light" || themeRaw === "dark" ? themeRaw : "auto";
  const heightRaw = Number.parseInt(params.get("h") || "", 10);
  const height = Number.isFinite(heightRaw) ? Math.max(320, Math.min(800, heightRaw)) : 400;
...
export function toSearchParams(state: ShareState): URLSearchParams {
  ...
  params.set("tg", state.tmode === "static" ? "0" : round(state.tg, 0.001).toFixed(3));
