 ...
 */
const DATA_SOURCE_URL_GENERATORS: Record<string, (sourceCode: string | null) => string | null> = {
 ...
 */
export function getDataSourceUrl(source: string | null, sourceCode: string | null): string | null {
  if (!source) return null;

  const generator = Object.hasOwn(DATA_SOURCE_URL_GENERATORS, source)
    ? DATA_SOURCE_URL_GENERATORS[source]
    : undefined;
...
export function getDataSourceBaseUrl(source: string | null): string | null {
  ...
  return Object.hasOwn(baseUrls, source) ? baseUrls[source] : null;
}
...
export function getWorldBankUrl(indicatorCode: string): string | null {
  if (!Object.hasOwn(WORLD_BANK_INDICATOR_CODES, indicatorCode)) {
    return null;
  }
  const wbCode = WORLD_BANK_INDICATOR_CODES[indicatorCode];
  return typeof wbCode === "string" && wbCode.length > 0
    ? `https://data.worldbank.org/indicator/${wbCode}`
    : null;
}
 ...
 */
export function getDataSourceLicense(source: string | null): { name: string; url: string } | null {
  if (!source) return null;
  return Object.hasOwn(DATA_SOURCE_LICENSES, source) ? DATA_SOURCE_LICENSES[source] : null;
}
